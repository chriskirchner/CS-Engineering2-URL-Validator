...
//    }
    ...
    public void testValidURL(){
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader("validURLs.txt"));
			String line;
			while ((line = br.readLine()) != null) {
				assertEquals(urlValidator.isValid(line), refUrlValidator.isValid(line));
			}
			br.close();
		...
		/***************************
		**CODE TO WRITE PASSING TESTS TO FILE
		String[] validURLs = new String[1000];
		int numValid = 0;
		
        for (int i=0; i<1000; i++){
            String url = getURI(true);
			if (urlValidator.isValid(url) == refUrlValidator.isValid(url)){
				validURLs[numValid] = url;
				numValid++;
			}
        }
		
		try{
			//Write invalid URLS to file invalidURLs.txt
			 File file = new File("validURLs.txt");
			...
			//write invalidURLs to file
			for (int i = 0; i < numValid; i++){
				writer.write(validURLs[i] + "\n");
			}
    ...
    public void testInvalidURL(){
		try {
			BufferedReader br = new BufferedReader(new FileReader("invalidURLs.txt"));
			String line;
			while ((line = br.readLine()) != null) {
				assertEquals(urlValidator.isValid(line), refUrlValidator.isValid(line));
			}	
		...
		/*****************************************
		**CODE TO WRITE PASSING TESTS TO FILE
		String[] invalidURLs = new String[1000];
		int numInvalid = 0;
		
        for (int i=0; i<1000; i++){
            String url = getURI(false);
			if (urlValidator.isValid(url) == refUrlValidator.isValid(url)){
				invalidURLs[numInvalid] = url;
				numInvalid++;
			}
        }
		try{
			//Write invalid URLS to file invalidURLs.txt
			 File file = new File("invalidURLs.txt");
    ...
    public void testStuff(){
        String url = "bE:///#]?'?]:]//";
        System.out.println(refUrlValidator.isValid(url));
